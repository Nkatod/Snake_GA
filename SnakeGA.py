...
def getSurvPopul(
    ...
    while len(newpopul) < nsurv:
        bot = np.random.random((agent_size, 3))
        newpopul.append(bot)
...
class Snake(gym.Env):
    ...
    def get_states(self):
        ...
        cellfree_left = 1.0
        cellfree_right = 1.0
        cellfree_forward = 1.0
        cell2free_left = 1.0
        cell2free_right = 1.0
        cell2free_forward = 1.0

        if self.direction == 'RIGHT':
            if self.snakePos[0] < self.foodPos[0]:
                isfood_forward = 1.0
            elif self.snakePos[0] > self.foodPos[0]:
                isfood_right = 1.0
                isfood_left = 1.0
            if self.snakePos[1] < self.foodPos[1]:
                isfood_right = 1.0
            elif self.snakePos[1] > self.foodPos[1]:
                isfood_left = 1.0
            if self.snakePos[0] + 1 >= self.height:
                cellfree_forward = 0.0
            else:
                cellfree_forward = self.gameMatrix[self.snakePos[0] + 1][self.snakePos[1]]
            if self.snakePos[1] - 1 < 0:
                cellfree_left = 0.0
            else:
                cellfree_left = self.gameMatrix[self.snakePos[0]][self.snakePos[1] - 1]
            if self.snakePos[1] + 1 >= self.height:
                cellfree_right = 0.0
            else:
                cellfree_right = self.gameMatrix[self.snakePos[0]][self.snakePos[1] + 1]
            if self.snakePos[0]+2 >= self.height:
                cell2free_forward = 0.0
            else:
                cell2free_forward = self.gameMatrix[self.snakePos[0]+2][self.snakePos[1]]
            if self.snakePos[1] - 2 < 0:
                cell2free_left = 0.0
            else:
                cell2free_left = self.gameMatrix[self.snakePos[0]][self.snakePos[1] - 2]
            if self.snakePos[1] + 2 >= self.height:
                cell2free_right = 0.0
            else:
                cell2free_right = self.gameMatrix[self.snakePos[0]][self.snakePos[1] + 2]
        ...
        if self.direction == 'UP':
            ...
            if self.snakePos[1] - 1 < 0:
                ...
                cellfree_forward = self.gameMatrix[self.snakePos[0]][self.snakePos[1]-1]
            if self.snakePos[0] - 1 < 0:
                cellfree_left = 0.0
            else:
                cellfree_left = self.gameMatrix[self.snakePos[0] - 1][self.snakePos[1]]
            if self.snakePos[0] + 1 >= self.height:
                cellfree_right = 0.0
            else:
                cellfree_right = self.gameMatrix[self.snakePos[0] + 1][self.snakePos[1]]
            if self.snakePos[1] - 2 < 0:
                cell2free_forward = 0.0
            else:
                cell2free_forward = self.gameMatrix[self.snakePos[0]][self.snakePos[1]-2]
            if self.snakePos[0] - 2 < 0:
                cell2free_left = 0.0
            else:
                cell2free_left = self.gameMatrix[self.snakePos[0] - 2][self.snakePos[1]]
            if self.snakePos[0] + 2 >= self.height:
                cell2free_right = 0.0
            else:
                cell2free_right = self.gameMatrix[self.snakePos[0] + 2][self.snakePos[1]]
        ...
        state_dict = np.array([isfood_left,
                               isfood_forward,
                               isfood_right,
                               cellfree_left,
                               cellfree_right,
    ...
    def check_game_over(self):
        ...
        for block in self.snakeBody[1:]:
            if self.snakePos[0] == block[0] and self.snakePos[1] == block[1]:
                ...
                self.score -= 5
        self.energy -= 1
        if self.current_step > self.max_episode_steps or self.energy <= 0:
            self.game_over = True
    ...
    def reset(self):
        ...
        self.score = 0
        self.current_step = 0
        self.setFPSTick(5000)

    def setFPSTick(self, FPS):
        self.fpsController = self.pygame.time.Clock()
    ...
    def updateGameMatrix(self):
        for i in range(self.gameMatrix.shape[0]):
            for j in range(self.gameMatrix.shape[1]):
                self.gameMatrix[i][j] = 0.75
        # update snake pos
        for i in self.snakeBody:
            self.gameMatrix[i[0]][i[1]] = 0.40;
        self.gameMatrix[self.snakePos[0]][self.snakePos[1]] = 0.00
        self.gameMatrix[self.foodPos[0]][self.foodPos[1]] = 1.0
    ...
    def get_direction_from_GA(self):
        ...
        result = np.dot(states, self.agent)
    ...
    for it in range(epohs):  # создали список списков всех значений по эпохам
        ...
        for bot in popul:
            ...
            if showGame:
                ...
                snake_game.setFPSTick(FPS=1)
                showGame = False
            else:
                snake_game.mode = 'auto'
